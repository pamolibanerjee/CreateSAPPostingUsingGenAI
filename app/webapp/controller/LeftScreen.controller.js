        ...
        onProcessDocument: function(oEvent){
            ...
            this.requestDocumentProcessing(pdfFileID)
                .then((oReturn) => {
                    ...
                    MessageToast.show(oReturn);

                    // var oModel_JSON = new sap.ui.model.json.JSONModel();
                    // oModel_JSON.setData(JSON.parse(oReturn.value));
                    // sap.ui.getCore().setModel(oModel_JSON, 'oModel_JSON');
                })
                .catch((error) => {
                    this.byId("fileManagementFragment").setBusy(false);
